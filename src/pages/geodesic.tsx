...
}, [location.pathname, cleanup, resetRotation]);
...
const computeSphericalGeodesic = useCallback((startPoint: Point3D, endPoint: Point3D): GeodesicResult => {
  ...
  return { 
    path, 
    length: length * (EARTH_RADIUS / SPHERE_RADIUS),
    straightDistance: straightDistance * (EARTH_RADIUS / SPHERE_RADIUS),
...
const handlePointClick = useCallback((index: number, e: React.MouseEvent) => {
  ...
  if (!isDragging) {
    setSelectedPoints(prev => {
      const newPoints = prev.includes(index)
      ...
      if (newPoints.length === 2) {
        ...
        const result = computeSphericalGeodesic(
          points[newPoints[0]],
          points[newPoints[1]]
        );
        setGeodesic(result);
...
}, [isDragging, generateSpherePoints, computeSphericalGeodesic]);

const basePoints = generateSpherePoints();
const points = basePoints.map(point => {
  const projected = rotatePoint(point);
  return { ...projected, original: point };
});
if (location.pathname !== '/geodesic') {
  return null;
}
